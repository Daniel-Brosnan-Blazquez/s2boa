"""
Archive data volumes view for s2boa
...
def show_archive_data_volumes():
    ...
    """
    current_app.logger.debug("Archive data volumes view")
    ...
    show = {}
    define_what_to_show_archive_data_volumes(show)
...
def query_archive_data_volumes_pages():
    ...
    """
    current_app.logger.debug("Archive data volumes view using pages")
...
def show_sliding_archive_data_volumes():
    ...
    if request.method == "POST":
        ...
        if request.form["archive_data_volumes_window_delay"] != "":
            window_delay = float(request.form["archive_data_volumes_window_delay"])
        # end if

        if request.form["archive_data_volumes_window_size"] != "":
            window_size = float(request.form["archive_data_volumes_window_size"])
        # end if

        if request.form["archive_data_volumes_repeat_cycle"] != "":
            repeat_cycle = float(request.form["archive_data_volumes_repeat_cycle"])
        # end if
...
def define_what_to_show_archive_data_volumes(show):
    ...
    if request.method == "POST":
        if not "show_archive_data_volumes_table_details" in request.form:
            show["table_details"] = False
        else:
            show["table_details"]=True
        # end if
        if not "show_archive_data_volumes_map" in request.form:
            show["map"] = False
        else:
            show["map"]=True
        # end if
        if not "show_archive_data_volumes_evolution" in request.form:
...
def query_datastrips_and_render(start_filter = None, stop_filter = None, mission = None, show = None, sliding_window = None, filters = None):
    ...
    return render_template("views/archive_data_volumes/archive_data_volumes.html", datastrip_events=datastrip_events, orbpre_events=orbpre_events, request=request, show=show, reporting_start=reporting_start, reporting_stop=reporting_stop, sliding_window=sliding_window, filters = filters)
...
def query_datastrip_events(start_filter = None, stop_filter = None, mission = None, filters = None):
    ...
    kwargs["gauge_names"] = {"filter": "TIMELINESS", "op": "=="}

    timeliness_events = query.get_events(**kwargs)
    explicit_refs = [event.explicitRef.explicit_ref for event in timeliness_events]
    
    events = query.get_events(explicit_refs = {"filter": explicit_refs, "op": "in"},
                              gauge_names = {"filter": "PROCESSING_VALIDITY", "op": "=="})
