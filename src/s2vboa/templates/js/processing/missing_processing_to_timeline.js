...
var missing_processing_timeline = [
    ...
    {% set sad_data = original_isp_validity.eventLinks|selectattr("name", "equalto", "SAD_DATA")|map(attribute='event_uuid_link')|list|first %}
    {
        ...
        "tooltip": create_processing_tooltip_text("{{ event.event_uuid }}", "{{ satellite }}", "{{ downlink_orbit }}", "{{ station }}", "{{ level }}", "{{ sensing_orbit }}", "<a class=bold-red>{{ status }}</a>", "/eboa_nav/query-event-links/{{ sad_data }}", "/eboa_nav/query-event-links/{{ datastrip }}"),
