...
var incomplete_processing_timeline = [
    {% for event in events %}
    ...
    {% endif %}
    {
        ...
        "tooltip": create_processing_tooltip_text("{{ event.event_uuid }}", "{{ satellite }}", "<a href='/views/specific-processing/{{ original_planned_playback.event_uuid }}'>{{ downlink_orbit }}</a>", "{{ station }}", "{{ level }}", "{{ sensing_orbit }}", "<span class=bold-orange>{{ status }}</span>", "{{ datastrip }}", "{{ datastrip_start }}", "{{ datastrip_stop }}", "{{ sad_data_info }}"),
