...
function create_playback_tooltip_text(satellite, orbit, start, stop, playback_type, playback_mean, plan_file, uuid, link_to_details){
...
var playback_events = [
    ...
    {% set playback_mean = event.eventTexts|selectattr("name", "equalto", "playback_mean")|map(attribute='value')|first|string %}
    {
        ...
        "tooltip": create_playback_tooltip_text("{{ satellite }}", "{{ orbit }}", "{{ event.start.isoformat() }}", "{{ event.stop.isoformat() }}", "{{ playback_type }}", "{{ playback_mean }}", "{{ original_playback.source.name }}", "{{ original_playback.event_uuid }}", "/eboa_nav/query-event-links/{{ original_playback.event_uuid }}")
    },
    {% endfor %}
]
