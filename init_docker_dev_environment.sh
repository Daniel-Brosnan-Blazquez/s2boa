...
USAGE="Usage: `basename $0` -e path_to_eboa_src -v path_to_vboa_src -d path_to_dockerfile -o path_to_orc_packets -u host_user_to_map -f path_to_eopcfi [-p port] [-t path_to_tailored] [-l containers_label] [-a app] [-c boa_tailoring_configuration_path] [-s path_to_boa_certificates] [-n] [-r]\n
Where:\n
...
while getopts e:v:d:o:u:p:t:l:a:c:s:nrf: option
do
    case "${option}"
        in
...
while true; do
    read -p "
Welcome to the initializer of the S2BOA development environment :-)
