...
sphinx-build -b latex ../../src/docs/s2boa/ build
# Add chapter part
echo "\chapter{Ingestion modules code documentation}" > s2boa_code_documentation.tex
# Extract the chapter describing the code
sed -n '/\\section{Subpackages}/,/\\section{Module contents}/p' build/S2BOA_module.tex |head -n -1 >> s2boa_code_documentation.tex

## S2VBOA
# Generate the tex file
sphinx-build -b latex ../../src/docs/s2vboa/ build
# Add chapter part
echo "\chapter{View modules code documentation}" > s2vboa_code_documentation.tex
# Extract the chapter describing the code
sed -n '/\\section{Subpackages}/,/\\section{Module contents}/p' build/S2VBOA_module.tex |head -n -1 >> s2vboa_code_documentation.tex
